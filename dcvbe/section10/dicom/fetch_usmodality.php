<?php
	...
	$response['testplan_dicom_usmodality'] = array() ;
	$testplan_dicom_usmodality = array() ;
	...
	if(isset($_GET['prs']))
	{
		...
		if($resultCheck>0)
		{
			...
			if($resultCheck>0)
			{
				...
				//Query Contact Table
				$sql = "SELECT *
						FROM section10_testplan_dicom_usmodality
				...
				$testplan_dicom_usmodality["initiate_query_pass"] = $row[2];
$testplan_dicom_usmodality["initiate_query_comments"] = $row[3];
$testplan_dicom_usmodality["locate_order_pass"] = $row[4];
$testplan_dicom_usmodality["locate_order_comments"] = $row[5];


					array_push($response["testplan_dicom_usmodality"], $testplan_dicom_usmodality) ;
